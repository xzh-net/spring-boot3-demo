 ...
 * 门户前端应用 (portal-app)
 * ------------------------------------
 * 纯前端服务, 不持有 client_secret, 通过调用 portal-server (8080) 后端 API 获取:
...
/** 代理请求到 portal-server (8080), 透传 Cookie */
...
const server = http.createServer(async (req, res) => {
    ...
    // API 代理: 将 /api/* 请求转发到 portal-server (8080)
    if (pathname.startsWith('/api/')) {
        try {
            ...
            res.end(JSON.stringify({ error: 'portal-server 不可达: ' + e.message }));
        }
        return;
    }

    // 登录: 重定向到 portal-server 的 OAuth2 登录
    if (pathname === '/login') {
        res.writeHead(302, {
    ...
    // 登出: 重定向到 portal-server 的 OAuth2 登出
    if (pathname === '/logout') {
        res.writeHead(302, {
...
server.listen(PORT, () => {
    console.log(`[portal-app] 门户前端已启动: http://localhost:${PORT}`);
    console.log(`[portal-app] 后端服务地址: ${PORTAL_SERVER}`);
    console.log(`[portal-app] 认证中心地址: ${AUTH_SERVER}`);
});
