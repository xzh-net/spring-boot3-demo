...
| `portal-app` | 门户前端 (Node) | 8000 | 纯前端，展示客户端列表与 SSO 跳转卡片 | [README](./portal-app/README.md) |
| `portal-server` | 门户 BFF (Java) | 8080 | OAuth2 Client (Confidential + PKCE)，持有 client_secret | [README](./portal-server/README.md) |
| `oauth2-callback-web-app` | 演示 (Node) | 8081 | 授权码 / 密码模式 — 传统 Web 应用 (Confidential Client) | [README](./oauth2-callback-web-app/README.md) |
| `oauth2-callback-device-app` | 演示 (Node) | 8082 | 设备码模式 — 输入受限设备 (RFC 8628) | [README](./oauth2-callback-device-app/README.md) |
| `oauth2-callback-mobile-app` | 演示 (Node) | 8083 | 授权码 + PKCE — 移动应用 / SPA (Public Client) | [README](./oauth2-callback-mobile-app/README.md) |
...
```bash
cd portal-server && mvn spring-boot:run   # http://localhost:8080  门户 BFF
cd portal-app && node server.js           # http://localhost:8000  门户前端
```

### 3. 启动演示项目（按需）

```bash
cd oauth2-callback-web-app     && node server.js   # http://localhost:8081  传统 Web 应用
cd oauth2-callback-device-app  && node server.js   # http://localhost:8082  设备码
cd oauth2-callback-mobile-app  && node server.js   # http://localhost:8083  移动应用 / SPA
```
...
- **前后端分离门户**：portal-app + portal-server BFF 架构
- **多环境配置**：dev / local / prod 通过 `spring.profiles.active` 切换

## 设计文档

- [统一认证管理平台设计说明书](./统一认证管理平台设计说明书.md)（V4.0 完整版，整合 V3.1 基线 + OIDC/PKCE SSO + 在线用户与会话管理 + 测试方案）
