 ...
 * 确保 redirect_uri 指向独立的 portal-server (8080) 而非认证中心本机。
 */
@Slf4j
@Component
...
public class DataInitializer implements ApplicationRunner {
     ...
     * portal-app 客户端密钥.
     * <p>
     * portal-server (8080) 的 application.yml 中 client-secret 必须与此值一致。
     */
    private static final String PORTAL_CLIENT_SECRET = "123456";

    /**
     * portal-server 的 OAuth2 回调地址.
     * <p>
     * Spring Security OAuth2 Client 默认回调路径: {baseUrl}/login/oauth2/code/{registrationId}
     * registrationId = portal-app-oidc (见 portal-server application.yml)
     * <p>
    ...
    private void ensurePortalAppClient() {
        try {
            ...
            if (!actualRedirects.containsAll(expectedRedirects)) {
                log.info("[DataInit] portal-app redirect_uri 需更新为 3 个 (portal-server/web-app/mobile-app)");
    ...
    private void createPortalAppClient() {
        try {
            ...
            RegisteredClient portalApp = RegisteredClient.withId("1")
                    ...
                    .redirectUri(PORTAL_REDIRECT_URI)           // portal-server 自己的 OAuth2 login 回调
