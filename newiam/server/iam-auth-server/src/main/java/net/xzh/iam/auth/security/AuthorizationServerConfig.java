...
import net.xzh.iam.auth.security.repository.RedisOAuth2AuthorizationService;
import net.xzh.iam.auth.security.authentication.client.DeviceClientAuthenticationProvider;
import net.xzh.iam.auth.security.authentication.grant.DeviceCodeGrantAuthenticationProvider;
import net.xzh.iam.auth.security.authentication.grant.PasswordGrantAuthenticationProvider;
import net.xzh.iam.auth.security.authentication.grant.PolicyAwareAuthorizationCodeAuthenticationProvider;
import net.xzh.iam.auth.security.authentication.grant.PolicyAwareRefreshTokenAuthenticationProvider;
import net.xzh.iam.auth.security.session.RedisSessionRegistry;
import net.xzh.iam.auth.security.token.EnrichedOAuth2TokenIntrospectionAuthenticationProvider;
import net.xzh.iam.auth.security.token.RedisOpaqueTokenIntrospector;
import net.xzh.iam.auth.security.web.ActiveClientTrackingFilter;
import net.xzh.iam.auth.security.web.AuthorizePolicyFilter;
import net.xzh.iam.auth.security.web.CompositeSecurityContextRepository;
import net.xzh.iam.auth.security.web.SessionExpirationFilter;
import net.xzh.iam.auth.security.web.converter.DeviceClientAuthenticationConverter;
import net.xzh.iam.auth.security.web.converter.PasswordGrantAuthenticationConverter;
 ...
 * <li><b>Order(2)</b> — 身份供给内部 API 链：/api/internal/identity/**（M2M 服务令牌 + 服务身份白名单，
 *     供身份管理面 iam-identity-service 调用；对外管理入口已迁往管理面）</li>
...
public class AuthorizationServerConfig {
	 ...
	 * 身份供给内部 API 安全链。
	 * <p>
	 * 保护 {@code /api/internal/identity/**} 内部供给接口（用户 / 客户端 / 会话 / 授权记录 / 租户）。
	 * 仅接受 {@code authserver.internal-identity-client-ids} 白名单内 M2M 服务身份
	 * (client_credentials, 当前即身份管理面 iam-identity-service 所用的 admin-m2m)
	 * 签发的服务令牌访问; 持用户令牌 (authorization_code) 的调用方即使具备管理角色也被拒绝——
	 * 对外管理入口已迁往 iam-identity-service, 认证中心不再直接暴露管理面。
	 */
	@Bean
	@Order(2)
	public SecurityFilterChain identityInternalApiSecurityFilterChain(HttpSecurity http,
			RedisOpaqueTokenIntrospector redisOpaqueTokenIntrospector) throws Exception {
		http.securityMatcher("/api/internal/identity/**")
				// 授权决策: 令牌须为白名单内 M2M 服务身份签发 (client_credentials)
				.authorizeHttpRequests(auth -> auth
						.requestMatchers("/api/internal/identity/**")
						.access(internalIdentityAccessManager(authServerProperties.getInternalIdentityClientIds())))

				// OAuth2 资源服务器: 本地 Redis 自省器校验 Bearer Token, 无远程 introspection 依赖
				.oauth2ResourceServer(rs -> rs.opaqueToken(token -> token.introspector(redisOpaqueTokenIntrospector)))

				// 认证/授权失败分别以 JSON 401 / 403 返回
				.exceptionHandling(ex -> ex
...
/**
 * 内部 API 访问决策：请求令牌须为白名单内 M2M 服务身份 (client_credentials) 签发,
 * 以内省属性 client_id 裁决; 用户令牌即使具备管理角色也不放行。
 */
private static AuthorizationManager<RequestAuthorizationContext> internalIdentityAccessManager(Set<String> internalClientIds) {
    return (authentication, context) -> {
        ...
        if (auth.getPrincipal() instanceof OAuth2AuthenticatedPrincipal principal) {
            ...
            Object grantType = principal.getAttributes().get("grant_type");
            if (clientId != null && internalClientIds.contains(clientId.toString())
                    && "client_credentials".equals(String.valueOf(grantType))) {
	 ...
	 * 本站同源路径 (以 / 开头, 不包含 // 协议跳) 或 OAuth2 回调客户端地址 (新端口段)。
	 */
	private static final Set<String> ALLOWED_REDIRECT_HOSTS = Set.of(
			"localhost:8080", "localhost:8100", "localhost:8101", "localhost:8200", "localhost:8201",
			"localhost:8300", "localhost:8301", "localhost:8302", "localhost:9000",
			"127.0.0.1:8080", "127.0.0.1:8100", "127.0.0.1:8101", "127.0.0.1:8200", "127.0.0.1:8201",
			"127.0.0.1:8300", "127.0.0.1:8301", "127.0.0.1:8302", "127.0.0.1:9000"
	);
