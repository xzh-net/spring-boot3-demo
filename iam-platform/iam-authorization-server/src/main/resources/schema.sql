...
-- 典型场景: iam-portal-service (后端 8080) 作为 OAuth2 Client, 通过授权码流程与认证中心交互
--   iam-portal-web (前端 8000) 为纯前端, 通过 iam-portal-service 后端代理完成 token 交换
--   client_authentication_method=client_secret_basic (Confidential Client, 密钥 123456)
--   redirect_uri 指向 iam-portal-service 的标准回调路径: {baseUrl}/login/oauth2/code/{registrationId}
...
-- redirect_uris 支持:
--   Node.js 演示 (iam-client-web-demo, 端口 8081) -> /callback
