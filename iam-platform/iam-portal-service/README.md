...
门户后端服务（Backend For Frontend），作为 OAuth2 Client 与认证中心交互，为 iam-portal-web 前端提供 REST API。
...
[浏览器] ──► iam-portal-web (8000, 前端)
                 ...
                 │  /api/* 代理（透传 Cookie）
                 ▼
             iam-portal-service (8080, BFF)  ← 本模块
                 │
...
允许 `http://localhost:8000`（iam-portal-web 前端）跨域调用，允许携带 Cookie (credentials)。
...
- [iam-portal-web README](../iam-portal-web/README.md)
